# Pruebas unitarias AISLADAS (sin base de datos) del RBAC del CU9 (Gestionar categorias).
# Regla: la GESTION (POST/PUT/DELETE) es solo de ASU; la CONSULTA (GET) no cambia.
# Sin token -> 401 en gestion; cualquier otro rol (incl. Cliente) -> 403. ASU supera la autorizacion.
# Ejecutar desde la raiz del backend:  python tests/unit/test_cu9_rbac.py
import sys
import unittest
import uuid
...
CAT = "/api/v1/categorias"
BODY = {"nombre": "Camisas", "linea": "Hombre"}

# Operaciones de GESTION existentes del CU9
GESTION = [
    ("crear", "POST", CAT, BODY),
    ("crear (barra)", "POST", CAT + "/", BODY),
    ("editar / activar / desactivar", "PUT", f"{CAT}/1", {"activo": False}),
    ("eliminar", "DELETE", f"{CAT}/1", None),
]
...
class CU9SoloASUGestiona(unittest.TestCase):
    def setUp(self):
    ...
    def test_403_antes_de_validar_el_body(self):
        self.como("C")
        self.assertEqual(self.client.post(CAT, json={}).status_code, 403)
